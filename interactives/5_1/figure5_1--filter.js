...
function handleFilterTransitions(data, barType, sector, dataType, svg, rects, x, y, xAxis, yAxis, changeBarType) {
    var max_area = (dataType === "area") ? ( (barType === "stacked") ? 1809124505200 : 1193e9 ) : 1;
    ...
    if (changeBarType === true) {
        if (barType === "stacked") {
            ...
            rects.transition()
                .duration(300)
                    .attr("y", function(d, i) { return y(d.data.region); })
                    .attr("height", y.bandwidth())
                .transition()
                    .attr("x", function(d, i) { return x(0); })
        }
        ...
        if (barType === "stacked") {
            rects.data(stackedData, function (d) { return d.data.id;})
                ...
                .transition(xt)
                    .delay(300)
                    .attr("x", function(d) { return x(d[0]); })
                    .attr("width", function(d) { return d.data.type === sector ? x(d[1]) - x(d[0]) : 0 })
                    .attr("height", y.bandwidth())
        } else if (barType === "grouped") {
            rects.transition(t)
                .attr("x", function (d, i) { return x(0); })
...
var initStackedBarChart = {
    draw: function(config) {
        ...
        function triggerTypeReorder() {
            ...
            this.classList.add("active");
//            console.log(rects)
            handleFilterTransitions(data, barType, sector, dataType, svg, rects, x, y, xAxis, yAxis, false);
        }
